...
void testPnp() {
  ...
  rai::Frame& obj =
      ...
      .setShape(rai::ST_ssBox, {.06,.15,.15,.01})
      .setRelativePosition(arr{.2,.2,.08});
  C.watch(true);
...
//  FrankaGripper G((leftRight=="right"?0:1));
  ...
  komo.addObjective({1.}, FS_positionDiff, {"R_gripperCenter", "target"}, OT_eq, {1e2});
...
//    komo.addObjective({1.,1.2}, FS_position, {"R_gripperCenter"}, OT_eq, {}, {0.,0.,.5}, 2, +1, +0);
  ...
  komo.addObjective({2.}, FS_positionDiff, {"target", "table"}, OT_eq, {1e2}, {0.,0.,.08});
...
//    komo.addObjective({1.1,2.1}, FS_position, {"R_gripperCenter"}, OT_eq, {}, {0.,0.,.1}, 2);
  ...
  komo.addObjective({3.}, FS_qItself, {}, OT_eq, {}, q0);
...
//  sp->moveTo(q0, 1., ctrlTime, true);
  ...
  for(uint k=0;k<path.N;k++){
    //send komo as spline:
    {
      arr times = range(0., 5., path(k).d0-1);
      times += times(1);
      double ctrlTime = robot.state.get()->time;
