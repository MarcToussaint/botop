...
int main(int argc, char** argv) {
...

  {
    ...
    if(rai::getParameter<bool>("sim", true)){
      processes.append( make_shared<ControlEmulator>(ctrl_ref, ctrl_state, ctrl_config.get()) );
    }else{
    ...
    {
      auto t = addCtrlTask(ctrl_tasks, ctrl_config, "upDown", FS_position, {"endeffL"}, .1);
      arr y0 = t->y;
      t->setTarget( y0 + ARR(.0, .0, .2) );

      auto t2 = addCtrlTask(ctrl_tasks, ctrl_config, "upDown", FS_position, {"endeffR"}, 1.);
      arr y2 = t2->y;
